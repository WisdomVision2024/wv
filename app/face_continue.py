...
def facecon(data):
    ...
    # print("b:"+sec.text)
    sec = geminimodule.model.generate_content("請幫我檢查result中的項目，並比較它們是否與content中的項目相同。如果result中的項目與content中的某個項目相符，請回答該項目出現在您的視野範圍。例如：如果result是Paney、Huang，而content是Paney，請回答「Paney出現在您的視野範圍」。如果result中沒有任何符合content的項目或result為空，則回答「  」，請注意不要出現其他回答。以下是正式數據：result："+result+"。content："+content+"。")
