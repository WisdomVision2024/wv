    ...
    def ask(self,question):
            ...
            if keyword in self.distribution:
                ...
                while True:
                    with open('img_object.txt', 'r') as file:
                        content = file.read()  # 去除空白字符，避免空行干擾
  
                    if content:  # 如果內容不為空
                        break  # 跳出循環，繼續下一步
                    elif time.time() - start_time > timeout:  # 超時檢查
                ...
                response = model.generate_content("現在"+x+"，此為使用者前方的畫面，請根據此回答問題，請回答完整句子，如果畫面中沒有該物品，就請回答沒有。接下來為問題:"+question)
                return(response.text)
        #人物類
        for keyword in keywordc:
            if keyword in self.distribution:
                ...
                x=face.face(content)
                # 輸入結果
                response = model.generate_content("使用者的前方有:"+x+"此為使用者前方的辨識出的人，請根據此回答問題，請回答完整句子，回答問題時請以「您」稱呼使用者，如果畫面中沒有該人，就請回答沒有。例如:使用者詢問:我的前方有誰。請回答:您的前方有(畫面中出現的人)。接下來為問題:"+question)
                return(response.text)
        #其他類
